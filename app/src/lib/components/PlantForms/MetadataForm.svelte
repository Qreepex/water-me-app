	...
	<div class="space-y-4">
		<fieldset>
			<legend class="mb-2 block text-base font-semibold text-gray-700">
				{$tStore('plants.formSafety')}
			</legend>
			<label class="flex min-h-11 items-center gap-3">
				<input type="checkbox" bind:checked={formData.isToxic} class="h-5 w-5" />
				<span class="text-base text-gray-700">{$tStore('plants.formToxicToPetsChildren')}</span>
			</label>
		</fieldset>

		<div>
			<span class="mb-2 block text-base font-semibold text-gray-700">{$tStore('plants.flags')}</span
			>
			<div class="space-y-2">
		...
		<fieldset>
			<legend class="mb-2 block text-base font-semibold text-gray-700"
				>{$tStore('plants.notesTitle')}</legend
			>
			<div class="mb-2 flex flex-col gap-2 sm:flex-row">
				<input
					...
					placeholder={$tStore('plants.formAddNotePlaceholder')}
			...
			{:else}
				<p class="text-base text-gray-500 italic">{$tStore('plants.formNoNotesYet')}</p>
			{/if}
		</fieldset>
	</div>
</div>
